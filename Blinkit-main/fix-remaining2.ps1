...
# ===== FIX A: categories_screen.dart - View Cart bar with product thumbnails =====
...
class _CategoriesScreenState extends State<CategoriesScreen> {
  ...
  Widget _buildCartThumbnails(CartProvider cart) {
    final images = cart.items.map((item) => _cartImageUrl(item)).take(2).toList();
...
Write-Host "OK: categories_screen.dart updated - View cart bar now shows product thumbnails" -ForegroundColor Green

# ===== FIX B: product_card.dart - remove OFF badge and Out of stock badge =====
$productCardContent = @'
import 'package:flutter/material.dart';
...
Write-Host "OK: product_card.dart updated - OFF badge and Out of stock badge removed" -ForegroundColor Green
